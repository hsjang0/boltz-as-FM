...
class TemplateModule(nn.Module):
    ...
    def forward(
        ...
        v = self.z_proj(self.z_norm(z[:, None])) + a_tij[:,:,1:,1:,:]
...
class MSAModule(nn.Module):
    ...
    def forward(
        ...
        has_deletion = feats["has_deletion"].unsqueeze(-1)[:,:,1:]
        deletion_value = feats["deletion_value"].unsqueeze(-1)[:,:,1:]
        is_paired = feats["msa_paired"].unsqueeze(-1)[:,:,1:]
        msa_mask = feats["msa_mask"][:,:,1:]
        token_mask = feats["token_pad_mask"].float()[:,1:]
