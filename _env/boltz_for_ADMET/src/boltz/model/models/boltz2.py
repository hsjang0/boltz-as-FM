...
class Boltz2(LightningModule):
    ...
    def forward( 
        ...
        with torch.set_grad_enabled(
            ...
            #print(z_init.shape)
            z_init = z_init #(z_init.shape)
            
            relative_position_encoding = self.rel_pos(feats)
            z_init = z_init + relative_position_encoding[:,1:,1:,:]
            z_init = z_init + self.token_bonds(feats["token_bonds"].float())[:,1:,1:,:]
            if self.bond_type_feature:
                z_init = z_init + self.token_bonds_type(feats["type_bonds"].long())[:,1:,1:,:]
            z_init = z_init + self.contact_conditioning(feats)[:,1:,1:,:]
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> dict:
        ...
        pred_dict["s"] = out.get("s")
        pred_dict["z"] = out.get("z")
        #print(pred_dict)
        return pred_dict
