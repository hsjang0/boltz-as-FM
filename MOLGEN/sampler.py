...
class Sampler_mol(object):
    ...
    def sample(self):
        ...
        with open(f'data/{self.configt.data.data.lower()}_train_nx.pkl', 'rb') as f:
            self.train_graph_list = pickle.load(f)                                   # for init_flags
        ...
        for r in range(num_sampling_rounds):
            ...
            if not(os.path.isdir(f'/cache/hsjang/samples/mols/{self.log_folder_name}')):
                os.makedirs(os.path.join(f'/cache/hsjang/samples/mols/{self.log_folder_name}'))
            with open(os.path.join('/cache','hsjang','samples','mols', f'{self.log_folder_name}', f'{self.log_name}.txt'), 'a') as f:
                for smi in smiles:
                    f.write(f'{smi}\n')
        ...
        scores = get_all_metrics(gen=gen_smiles, k=len(gen_smiles), n_jobs=24, test=test_smiles)

        for metric in ['FCD/Test', 'Scaf/Test', 'Frag/Test', 'SNN/Test', f'unique@{len(gen_smiles)}', 'valid']:
