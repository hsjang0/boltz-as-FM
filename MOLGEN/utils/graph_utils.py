...
def mask_adjs(adjs, flags):
    ...
    if adjs.shape[-1] >= 128: # B X N X N X C
        adjs = adjs * flags.unsqueeze(-1).unsqueeze(-1) # B x N x 1 x 1
        adjs = adjs * flags.unsqueeze(-2).unsqueeze(-1) # B x 1 x N x 1
        return adjs
...
def rand_perm(x, adj, emb, emb2):
    ...
    pemb = emb
    pemb = torch.bmm(peye, pemb.reshape(pemb.shape[0], 38, 38*emb.shape[-1])).reshape(pemb.shape).permute(0,3,1,2)
    pemb = torch.bmm(pemb.reshape(pemb.shape[0], 38*emb.shape[-1], 38), peye.transpose(-1,-2)).reshape(pemb.shape)
    
    pemb2 = torch.bmm(peye, emb2)
    #print(pemb.permute(0,2,3,1).shape)
    return px, padj, pemb.permute(0,2,3,1), pemb2
