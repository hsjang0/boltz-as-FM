...
def load_batch(batch, device):
    device_id = torch.device(f'cuda:{device[0]}' if isinstance(device, list) else device)
    x_b, adj_b, emb_b, emb_b2 = batch
    if device_id.type != 'cuda':
        return (x_b.to(device_id), adj_b.to(device_id),
                emb_b.to(device_id), emb_b2.to(device_id))

    copy_stream = torch.cuda.Stream(device=device_id)
    with torch.cuda.stream(copy_stream):
        x_b = x_b.to(device_id, non_blocking=True)
        adj_b = adj_b.to(device_id, non_blocking=True)
        emb_b = emb_b.to(device_id, non_blocking=True)
        emb_b2 = emb_b2.to(device_id, non_blocking=True)
    torch.cuda.current_stream(device_id).wait_stream(copy_stream)
    return x_b, adj_b, emb_b, emb_b2
...
def load_model_params(config):
    ...
    params = {'model_type': config.model.type, 'n_layers': config.model.num_layers,  
                ...
                'repa_depth': config.train.repa_depth,
                'repa_hidden': config.train.repa_hidden}
...
def load_ckpt(config, device):
    ...
    ckpt_dict = torch.load(path, map_location=device_id, weights_only=False)
    print(f'{path} loaded')
    return ckpt_dict
...
def load_model_from_ckpt(params, state_dict, device):
    ...
    state_dict = {k: v for k, v in state_dict.items() if k.count('repa_')==0}
